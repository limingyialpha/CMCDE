...
import logic.data.Utility.{cumulative_sum, empirical_variance, variance}
/**
...
trait McdeStats{
  ...
  val M: Int
  var parallelize: Int
  val contrast_method_shortnames = Set("R", "ItR", "ItGR", "ItGI","ItGIbV")
  ...
  def contrast(m: PreprocessedData, dimensions: Set[Int], MC_num:Int = M)(estimator: String = "R", slice_technique: String = "c"): Double = {
    estimator match {
      case "R" => contrast_random_ref_dim(m,dimensions.toSet, MC_num)(slice_technique)
      ...
      case "ItGIbV" => contrast_iterate_ref_dim_tail_group_ref_dim_by_influence_balance_with_variance(m, dimensions,MC_num)(slice_technique)
    }
  }
 ...
 //dimensions starts from 0
  ...
  def contrast_iterate_ref_dim_tail_group_ref_dim_by_influence_balance_with_variance(m: PreprocessedData, dimensions: Set[Int], MC_num:Int = M)(slice_technique: String = "c"): Double = {
    if (MC_num == 0) -1 else {
      ...
      val (tail_contrast, beta) = if (tail_MC_number >= 1) {
        ...
        val beta = if (head_great_iteration_num <= 1) (tail_MC_number.toDouble / MC_num) else {
          ...
          val var_tail = tail_bins_sizes.indices.map(b => {
            ...
            empirical_variance(influences) * scala.math.pow(current_bin_size.toDouble / num_dims, 2)
          }).sum

          // this happens in perfectly linear case
          if ((var_head == 0.0) & (var_tail == 0.0)) {tail_MC_number.toDouble / MC_num} else {
  ...
  def generalized_contrast(m: PreprocessedData, groups_of_dims: Set[Set[Int]], MC_num:Int = M)(slice_technique: String = "c"): Double = {
    if (MC_num == 0) -1 else {
      ...
      // Map(0 -> 0, 5 -> 2, 1 -> 0, 6 -> 2, 2 -> 1, 3 -> 1, 4 -> 2)
      val dim_group_num_dict = groups_of_dims.zipWithIndex.toVector flatMap{
        case (g,i) => g.map(d => (d,i)).toVector
      } map {
        case(d,i) => (d -> i)
      } toMap
