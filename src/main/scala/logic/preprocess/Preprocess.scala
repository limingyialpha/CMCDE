...
/**
  * Encapsulate a few preprocessing steps (open a CSV file, compute the rank logic.index structure).
  */
object Preprocess extends Preprocessing {
  ...
  /**
    ...
    * @param excludeIndex Whether to exclude an logic.index (the first column) or not.
  ...
  /**
    * Get the columns names of a data set in a map, assigning the position logic.index (integer) to the corresponding name (string)
    * @param path Path of the file in the system.
    * @param header Number of lines to discard (header), by default 1.
    * @param separator Number of lines to discard (header), by default 1.
    * @return An array of strings, where each string is a column name. Names are in the original order.
  ...
  /**
    ...
    * @return A 2-D Array of 2-D Tuple, where the first element is the original logic.index, the second is its value (actually not in used for the KSPmr test)
    */
  ...
  /**
    * Return the rank logic.index structure for MWP, with adjusted ranks but no correction for ties.
    *
    * @param input A 2-D Array of Double (data set, column-oriented).
    * @return A 2-D Array of 2-D Tuple, where the first element is the original logic.index, the second is its rank.
    */
  def mwRank(input: Array[Array[Double]], parallelize: Int): Array[Array[(Int, Float)]] = {
    // Create an logic.index for each column with this shape: (original position, adjusted rank, original value)
  ...
  /**
    * Return the rank logic.index structure for MWP, with adjusted ranks AND correction for ties.
    *
    * @param input A 2-D Array of Double (data set, column-oriented).
    * @return A 2-D Array of 3-D Tuple, where the first element is the original logic.index, the second is its rank and the
