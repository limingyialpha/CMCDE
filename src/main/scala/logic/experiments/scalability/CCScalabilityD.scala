 ...
 */
case class CCScalabilityD(output_folder: String) extends Experiment(output_folder) {
  // data specific params
  val generator: (Int, Double, String, Int) => DataGenerator = Independent
  val noise = 0
  val dimensions_of_interest = Vector(2, 4, 6, 8, 10, 12, 14)
  ...
  def run(): Unit = {
    ...
    for (dim <- dimensions_of_interest) {
      ...
      val cpu_times = (1 to pre_run_repetitions).par.map(_ => {
