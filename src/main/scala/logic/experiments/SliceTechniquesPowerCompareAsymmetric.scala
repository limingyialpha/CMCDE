...
object SliceTechniquesPowerCompareAsymmetric extends Experiment {
  val dimensions_of_interest = Vector(4,6,8,10,16)
  val noise_levels = 30
        ...
        for (noise <- noises.par) {
          for (gen <- generators.par) {
            ...
            val comparison_contrasts = (1 to comparison_iteration_num).par.map(_ => {
              val original_data = generator_instance.generate(obs_num)
              val independent_asymmetry = Independent(dim/2, 0, "gaussian", 0).generate(obs_num)
              val data = original_data.zip(independent_asymmetry).map(tuple => tuple._1 ++ tuple._2)
              val indices_set = data(0).indices.toSet
              val preprocessed = new RankIndex(data)
              ksp.contrast(preprocessed, indices_set)(estimator, slice_technique)
            }).toVector
            val power = comparison_contrasts.count(c => (c > threshold95)).toDouble / comparison_iteration_num.toDouble
            val to_write = List(generator_instance.id+"_ias", dim, noise, slice_technique, power).mkString(",")
