 ...
 */
object my_vs_edouard_uniform extends Experiment {
  ...
  def run():Unit = {
    ...
    for {
      ...
      for{rep <- 1 to nrep}{
        ...
        val my_array = ksp.contrast_iterate_uniform_cumulative_average_vec(preprocessed,dim_indices, MC_num = MC_num)
        val my_to_write = (List(generator.id,"my",rep) ++ my_array).mkString(",")
        summary.direct_write(summaryPath,my_to_write)
        val edouard_array = ksp.contrast_iterate_uniform_cumulative_average_vec_edouard(preprocessed,dim_indices,MC_num)
        val edouard_to_write = (List(generator.id,"edouard",rep) ++ edouard_array).mkString(",")
        summary.direct_write(summaryPath,edouard_to_write)
      }
    }
    ...
    for {
      ...
      for{rep <- 1 to nrep}{
        ...
        val my_array = ksp.contrast_iterate_uniform_cumulative_average_vec(preprocessed,dim_indices, MC_num = MC_num)
        val my_to_write = (List(gen_id,"my",rep) ++ my_array).mkString(",")
        summary.direct_write(summaryPath,my_to_write)
        val edouard_array = ksp.contrast_iterate_uniform_cumulative_average_vec_edouard(preprocessed,dim_indices,MC_num)
        val edouard_to_write = (List(gen_id,"edouard",rep) ++ edouard_array).mkString(",")
        summary.direct_write(summaryPath,edouard_to_write)
      }
    }
  }
