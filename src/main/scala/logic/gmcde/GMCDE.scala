 ...
 */
case class GMCDE(parallelize: Int = 0, num_iteration: Int = 50) {
  val id = "GMCDE"
  val alpha = 0.5
  val slice_technique = "c"

  def contrast(data: Array[Array[Double]], dims_set: Set[Int])(estimator: String, slice_technique: String): Double = {
    val preprocessed = new RankIndex(data)
    val ksp = KSP(num_iteration, alpha, parallelize)
    ksp.contrast(preprocessed, dims_set, num_iteration)(estimator, slice_technique)
  }
